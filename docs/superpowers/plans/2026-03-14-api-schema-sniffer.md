...
**Goal:** Un DevTools Snippet JS generalizado que intercepta todo el tráfico de red de cualquier web app (fetch, XHR, WebSocket, SSE, GraphQL), extrae schemas recursivos, captura request bodies en POST/PUT/PATCH, persiste en localStorage, e incluye modo stealth para evadir detección.

**Architecture:** IIFE con un `CONFIG` editable al inicio. Cada protocolo es un módulo opcional activable/desactivable. Modo stealth preserva `.toString()` en todos los overrides para parecer código nativo. Persiste en `localStorage[CONFIG.storageKey]` tras cada captura. GraphQL se detecta automáticamente por URL + body y se agrupa por `operationName`. Request bodies se capturan como schema separado (`REQ POST /endpoint`).

**Tech Stack:** Vanilla JavaScript ES2020 (sin dependencias), Chrome DevTools Snippets, localStorage, Blob API.
...
wc -l tools/api-schema-sniffer.js
```

Expected: archivo existe, ~230 líneas

- [ ] **Step 3: Commit**

```bash
git add tools/api-schema-sniffer.js
git commit -m "feat: DevTools snippet generalizado para capturar schemas de APIs (fetch, XHR, WS, SSE, GraphQL)"
```
...
No hay tests automatizados — este script corre en contexto de browser.

- [ ] **Step 1: Cargar en DevTools**

1. Abrir cualquier web app con DevTools (F12)
2. **Sources → Snippets** → `+ New Snippet` → pegar → `Ctrl+S`
3. `Ctrl+Enter` para ejecutar

Expected en Console:
```
[Sniffer] Iniciado. 0 endpoints ya capturados. Sin filtro (captura todo).
[Sniffer] Comandos: exportSchemas() · showSchemas() · clearSchemas()
```

- [ ] **Step 2: Verificar captura fetch/XHR**

Navegar la app. Expected en Console:
```
[Sniffer] ✓ GET https://api.example.com/v2/orders/:id
[Sniffer] ✓ REQ POST https://api.example.com/v2/orders
```

- [ ] **Step 3: Verificar stealth (opcional)**

En Console:
```javascript
window.fetch.toString()
// Expected: "function fetch() { [native code] }"   ← parece nativo
// Sin stealth: "async function patchedFetch(..."   ← detectable
```

- [ ] **Step 4: Verificar persistencia**

```javascript
localStorage.getItem('sniffer_schemas') // → JSON con schemas
```

Recargar la página, re-ejecutar el snippet:
```
[Sniffer] Iniciado. N endpoints ya capturados.
```

- [ ] **Step 5: Verificar WebSocket (si la app usa WS)**

Al llegar mensajes WS:
```
[Sniffer] ✓ WS wss://ws.example.com/realtime (merge)
```

- [ ] **Step 6: Verificar SSE (si la app usa SSE)**

Al llegar eventos SSE:
```
[Sniffer] ✓ SSE https://api.example.com/events (merge)
```

- [ ] **Step 7: Verificar GraphQL (si la app usa GQL)**

```
[Sniffer] ✓ GQL https://api.example.com/graphql [GetOrderDetail]
[Sniffer] ✓ GQL https://api.example.com/graphql [ListOrders]
```

- [ ] **Step 8: Exportar y verificar output**
...
Expected: descarga `schemas-<hostname>-YYYY-MM-DD.json` con estructura:
```json
{
  "GET https://api.example.com/v2/orders/:id": { "id": "number", ... },
  "REQ POST https://api.example.com/v2/orders": { "total": "number", ... },
  "WS wss://ws.example.com/realtime": { "type": "string", "payload": { ... } },
  "GQL https://api.example.com/graphql [GetOrder]": { "order": { ... } }
}
```
...
Editar `CONFIG.domains` antes de ejecutar:
...
Si no se captura nada: verificar en **Network tab → Fetch/XHR** qué dominios usa realmente
el panel y actualizar `domains` en consecuencia.

## Notas de diseño

- **Stealth**: `Object.defineProperty` + preservar `.toString()` hace que los overrides sean
  indistinguibles de las funciones nativas para código de detección simple.
- **WS/SSE merge**: los canales en tiempo real pueden mandar distintos tipos de mensajes.
  Se hace merge superficial de schemas para acumular todos los campos vistos.
- **GraphQL**: se detecta por URL que contiene `/graphql` + body con campo `query`.
  Se agrupa por `operationName` — si la operación es anónima, queda como `[anonymous]`.
- **Request bodies**: se capturan como `REQ METHOD /endpoint` separado del response,
  para no mezclar lo que enviamos con lo que recibimos.
- **EventSource named events**: solo se captura el evento `message` (genérico). Eventos
  nombrados (`.addEventListener('order_arrived', ...)`) no se capturan automáticamente —
  si aparecen, se pueden agregar manualmente.
